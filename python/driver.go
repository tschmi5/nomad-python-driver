...
var (
	...
	// PluginConfig is the python driver factory function registered in the
	// plugin catalog.
	...
	taskConfigSpec = hclspec.NewObject(map[string]*hclspec.Spec{
		...
		"file_path":          hclspec.NewAttr("class", "string", false),
		"requirements_path":  hclspec.NewAttr("class_path", "string", false),
...
type TaskConfig struct {
	FilePath     string       `codec:"file_path"`
	RequirementsPath string   `codec:"requirements_path"`
...
func (d *Driver) buildFingerprint() *drivers.Fingerprint {
	...
	version, runtime, vm, err := pythonVersionInfo()
	if err != nil {
	...
	fp.Attributes["driver.python.runtime"] = pstructs.NewStringAttribute(runtime)
	fp.Attributes["driver.python.vm"] = pstructs.NewStringAttribute(vm)
...
func (d *Driver) StartTask(cfg *drivers.TaskConfig) (*drivers.TaskHandle, *drivers.DriverNetwork, error) {
	...
	if driverConfig.FilePath == "" {
		return nil, nil, fmt.Errorf("file_path must be specified")
	}

	absPath, err := GetAbsolutePath("python3")
	if err != nil {
		return nil, nil, fmt.Errorf("failed to find python3 binary: %s", err)
	}
...
func pythonCmdArgs(driverConfig TaskConfig) []string {
	...
	// Add the classpath
	if driverConfig.FilePath != "" {
		args = append(args, driverConfig.FilePath)
	}

	// Add the jar
	// if driverConfig.RequirementsPath != "" {
		// args = append(args, "-jar", driverConfig.RequirementsPath)
	// }
